...
- 夜間バッチは生成した候補を DynamoDB の `aiSuggestedDescription` / `aiSuggestedImageUrl` として保存し、`aiStatus` を `NeedsReview` に戻します。
...
## 変更点（Bedrock モデルアクセス廃止対応）

- **影響範囲**: モデルアクセスの有効化操作が不要に。承認フロー自体へ直接の影響はなし。
- **権限**: Lambda 実行ロールに **Bedrock 最小権限**を追加（Claude/Titan それぞれ個別 ARN 指定）。
- **監査**: CloudWatch Logs で `InvokeModel` 呼び出しを記録。不要モデルを使わせないために IAM/SCP の **Deny** ルールを推奨。
