...
| **AI補完済み** | 状態 | 不可 | AI完了で ○ |
| **公開状態** | 表示制御 | 不可 | 公開/非公開 |
| **表示情報** | 内容制御 | 不可 | 元情報/優先公開情報(AI補完情報) |
| **ID** | 識別 | 自動 | 8桁ゼロパディング |
| **更新日時** | メタ | 自動 | ISO8601 |
...
```typescript
{
  ...
  flags: {
    ...
    displaySource: boolean,  // true=元情報, false=優先公開情報
    sourceHash: string,
...
sequenceDiagram
    ...
    Editor->>Sheet: 選択行で公開ボタン押下
    Note over Editor,Sheet: 3つの選択肢:<br/>元情報で公開<br/>優先公開情報で公開<br/>公開取りやめ
    Sheet-->>GAS: カスタムメニュー実行
    GAS->>GAS: 公開状態・表示情報を設定
    GAS->>APIGW: POST /webhook（source+published+状態）
    APIGW->>Lambda: Webhook処理
    Lambda->>Lambda: 署名・タイムスタンプ検証
    Lambda->>Dynamo: 公開状態・表示情報を更新
    Lambda->>S3: menu.json 再生成
    Lambda-->>GAS: 200 OK
    GAS-->>Sheet: 完了通知
```
...
4. GAS Callback 受信: 「AI補完済み=○」＋公開列群を更新
5. スタッフが公開列を確認・必要なら修正
6. 修正時: onEdit で「公開状態」を自動クリア
7. 選択行で公開ボタン押下（3つの選択肢）:
   - 「元情報で公開」: 公開状態=公開、表示情報=元情報
   - 「優先公開情報(AI補完情報)で公開」: 公開状態=公開、表示情報=優先公開情報(AI補完情報)
   - 「公開取りやめ」: 公開状態=非公開
8. GAS: 対応する列を設定 → `POST /webhook` 呼び出し（source + published + 公開状態 + 表示情報 送信）
9. Lambda: DynamoDB 更新 → menu.json 再生成（flags.displaySource ベースで表示情報選択）
10. 公開完了
...
### POST /webhook
- 入力: `{ itemId, source, published, publishStatus, displayInfo }`
- 処理: Dynamo更新（公開状態・表示情報） → menu.json 再生成
- 応答: `{ ok: true, itemId }`
...
| ボタン | 対象 | 動作 |
|--------|------|------|
| AI補完依頼 | 選択行 | AI補完依頼=○ → POST /ai/request |
| 元情報で公開 | 選択行 | 公開状態=公開、表示情報=元情報 → POST /webhook |
| 優先公開情報(AI補完情報)で公開 | 選択行 | 公開状態=公開、表示情報=優先公開情報(AI補完情報) → POST /webhook |
| 公開取りやめ | 選択行 | 公開状態=非公開 → POST /webhook |
| 現状強制同期 | 選択行 | POST /sync/menu（デバッグ用） |
| 最新情報取得 | 全体 | GET /ai/result → シート更新 |
...
- 表示情報は flags.displaySource で制御（true=元情報、false=優先公開情報）
- 優先公開列編集時は「公開状態」を自動クリア
- 公開はボタン承認のみ（公開状態=公開、flags.publishApproved=true）
- 公開取りやめは「公開状態=非公開」で制御
...
- 公開承認（元情報/優先公開情報/取りやめ） → menu.json 反映が5分以内
- スマホでCLS/INP良好（LCP<2.5s）
- 誤操作防止（列変更でAPI呼び出しなし、ボタンのみ）
- 公開状態と表示情報の組み合わせが正しく反映される
