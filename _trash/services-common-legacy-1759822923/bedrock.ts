...
// Allow-list pattern (defensive) – keeps early detectable misconfiguration
...
function validateModelId(modelId: string) {
  if (!modelId) {
    throw new BedrockModelAccessError('modelId is empty (ENV not set?)');
  }
  const ok = ALLOWED_MODEL_PREFIXES.some((p) => modelId.startsWith(p));
  if (!ok) {
    console.warn('[bedrock] modelId not in allow-list (continuing but flagged)', { modelId });
  }
  return modelId;
}
...
export function logBedrockEnv() {
  console.info('[bedrock] env', {
    region: REGION,
    claude: DEFAULT_CLAUDE_ID,
    embedding: DEFAULT_EMBED_ID
  });
}
...
export { BedrockModelAccessError };
