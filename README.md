...
✅ **アクセス制限**: QRコードベースのTTL付きトークン認証（Gemini API不正利用防止）  
...
**注意**: `SECRET_KEY`（アクセストークン用）は初回QRコード生成時に自動生成されます。
...
1. スプレッドシートで `メニューアプリ` → `QRコード・URL表示` を選択
2. 有効期限（日数）を入力（デフォルト: 7日間）
3. QRコードとURLが表示される
4. QRコードを印刷して店内に掲示

### 7. 動作確認

1. 生成したQRコードからアクセス、またはURLを開く
...
| 関数 | 目的 |
|------|------|
| `doGet()` | Webアプリのエントリーポイント（HTML配信・トークン検証） |
...
| `showQRCodeDialog()` | QRコード・URL生成ダイアログ表示 |
| `validateAccessToken()` | アクセストークン検証（HMAC-SHA256） |
...
1. QRコードの有効期限が切れている可能性があります
2. スプレッドシートで `メニューアプリ` → `QRコード・URL表示` から新しいQRコードを生成
3. 新しいQRコードを店内に掲示
...
1. `SECRET_KEY` が未設定の場合は、初回生成時に自動生成されます
2. ダイアログで「はい」を選択して秘密鍵を生成してください
3. それでもエラーが出る場合は、Apps Script のログを確認
...
- [x] キャッシュ制御の最適化（CacheService導入）✅
- [x] アクセス制限（QRコードベースのTTL付きトークン）✅
- [ ] エラーハンドリングの強化
- [ ] AI_Logs の可視化ダッシュボード
